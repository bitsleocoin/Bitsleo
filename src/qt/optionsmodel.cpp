...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitsleoAmount"))
        settings.setValue("nAnonymizeBitsleoAmount", 1000);
    nAnonymizeBitsleoAmount = settings.value("nAnonymizeBitsleoAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBitsleoAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBitsleoAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBitsleoAmount:
            return QVariant(nAnonymizeBitsleoAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBitsleoAmount:
            nAnonymizeBitsleoAmount = value.toInt();
            settings.setValue("nAnonymizeBitsleoAmount", nAnonymizeBitsleoAmount);
            emit AnonymizeBitsleoAmountChanged(nAnonymizeBitsleoAmount);
            break;
        default:
            break;
        }
