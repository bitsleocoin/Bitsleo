...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitsleoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitsleoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitsleoAmount = strAnonymizeBitsleoAmount.remove(strAnonymizeBitsleoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitsleoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitsleoAmount*COIN) nMaxToAnonymize = nAnonymizeBitsleoAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitsleoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitsleoAmount));
        strAnonymizeBitsleoAmount = strAnonymizeBitsleoAmount.remove(strAnonymizeBitsleoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitsleoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
