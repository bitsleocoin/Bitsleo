...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Bitsleo.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Bitsleod.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeBitsleoamount=<n> " + _("Keep N Bitsleo anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bitsleo is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitsleo is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bitsleo version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Bitsleo server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitsleo") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Bitsleo to complete") << "\n";
    ...
    nAnonymizeBitsleoAmount = GetArg("-anonymizeBitsleoamount", 0);
    if(nAnonymizeBitsleoAmount > 999999) nAnonymizeBitsleoAmount = 999999;
    if(nAnonymizeBitsleoAmount < 2) nAnonymizeBitsleoAmount = 2;
    ...
    LogPrintf("Anonymize Bitsleo Amount %d\n", nAnonymizeBitsleoAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BTSL+1000 == (.1BTSL+100)*10
       10BTSL+10000 == (1BTSL+1000)*10
    */
